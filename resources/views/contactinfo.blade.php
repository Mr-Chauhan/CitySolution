@extends('layouts.FrontHome')

@section('main_content')
  <div class="breadcrumb">
    <h2>Contact</h2>
  </div>

  <div class="map">
  <div style="width: 100%"><iframe width="100%" height="600" src="https://maps.google.com/maps?width=100%&amp;height=600&amp;hl=en&amp;q=bhavnagar%2Cgujarat+(My%20Business%20Name)&amp;ie=UTF8&amp;t=&amp;z=7&amp;iwloc=B&amp;output=embed" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"><a href="https://www.maps.ie/draw-radius-circle-map/">Radius map tool</a></iframe></div><br />
  </div>

  <div class="container">
    <div class="row">
      <div class="col-lg-6">
        <div class="recent">
          <h3>Send us a message</h3>
        </div>
        <div id="sendmessage">Your message has been sent. Thank you!</div>
        <div id="errormessage"></div>

        {!! Form::open(['method'=>'POST','name'=>'frmcontact','id'=>'frmcontact','route'=>'contact.store','files'=>true,'autocomplete'=>'off'])!!}
                    <div class="form-group">
                        {!!Form::label('varFname','First name *')!!}
                        {!!Form::text('varFname',null,['class'=>'form-control','placeholder'=>'First name'])!!}
                        <div class="validation"></div>
                    </div>
                    <div class="form-group">
                        {!!Form::label('varLname','Last name *')!!}
                        {!!Form::text('varLname',null,['class'=>'form-control','placeholder'=>'Last name'])!!}
                        <div class="validation"></div>
                    </div>
                    <div class="form-group">
                        {!!Form::label('varEmail','Email *')!!}
                        {!!Form::email('varEmail',null,['class'=>'form-control','placeholder'=>'Email'])!!}
                        <div class="validation"></div>
                    </div>
                    <div class="form-group">
                        {!!Form::label('varPhone','Phone *')!!}
                        {!!Form::tel('varPhone',null,['class'=>'form-control','placeholder'=>'Phone'])!!}
                        <div class="validation"></div>
                    </div>
                    <div class="form-group">
                        {!!Form::label('varMessage','Message *')!!}
                        {!!Form::textarea('varMessage',null,['class'=>'form-control','maxlength'=>500, 'row'=>2])!!}
                        <div class="validation"></div>
                    </div>

                    {!!Form::submit('Submit',['class'=>'btn btn-primary  btn-lg'])!!}

                    {!!Form::close()!!}

        <!-- <form action="" method="post" role="form" class="contactForm">
          <div class="form-group">
            <input type="text" name="name" class="form-control" id="name" placeholder="Your Name" data-rule="minlen:4" data-msg="Please enter at least 4 chars" />
            <div class="validation"></div>
          </div>
          <div class="form-group">
            <input type="email" class="form-control" name="email" id="email" placeholder="Your Email" data-rule="email" data-msg="Please enter a valid email" />
            <div class="validation"></div>
          </div>
          <div class="form-group">
            <input type="text" class="form-control" name="subject" id="subject" placeholder="Subject" data-rule="minlen:4" data-msg="Please enter at least 8 chars of subject" />
            <div class="validation"></div>
          </div>
          <div class="form-group">
            <textarea class="form-control" name="message" rows="5" data-rule="required" data-msg="Please write something for us" placeholder="Message"></textarea>
            <div class="validation"></div>
          </div>

          <div class="text-center"><button type="submit" class="btn btn-primary btn-lg">Send Message</button></div>
        </form> -->
      </div>

      <!-- <div class="col-lg-6">
        <div class="recent">
          <h3>Get in touch with us</h3>
        </div>
        <div class="">
          <p>Nam liber tempor cum soluta nobis eleifend option congue nihil imperdiet doming id quod mazim placerat facer possim assum. Typi non habent claritatem insitam; est usus legentis in iis qui facit eorum.</p>
          <p>Nam liber tempor cum soluta nobis eleifend option congue nihil imperdiet doming id quod mazim placerat facer possim assum. Typi non habent claritatem insitam; est usus legentis in iis qui facit eorum.</p>

          <h4>Address:</h4>Little Lonsdale St, New York<br>
          <h4>Telephone:</h4>345 578 59 45 416</br>
          <h4>Fax:</h4>123 456 789
        </div>
      </div> -->
    </div>
  </div>
    <div class="clearfix">...</div>

  @endsection
  
@section('script')
<script src="{{asset('front/js/contact-us.js')}}"></script>

@endsection
